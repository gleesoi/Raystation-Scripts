...
def autoplanPBB():
    ...
    #runs in CPython 3.6 64bit ## now use 3.6 cPython for use tkinter #error log interpreter
    #Raystattion 12A
    #Summer 2021
    ...
    def autoplanPBF():
        ...
        try:
            ...
            top = Tk()
            top.withdraw()
            messagebox.showerror("Error - Auto Plan Breast 26Gy/5F v" + str(script_version), "No patient loaded. Please open a patient")
            exit()
        ...
        try:
            ...
            top = Tk()
            top.withdraw()
            messagebox.showerror("Error - Auto Plan Breast 26Gy/5F v" + str(script_version), "No case loaded. Please open a case")
            exit()
        try:
            ...
            top = Tk()
            top.withdraw()
            messagebox.showerror("Error - Auto Plan Breast 26Gy/5F v" + str(script_version), "No examination loaded. Please open an examination")
            exit()
        ...
        try:
            ...
            top = Tk()
            top.withdraw()
            messagebox.showerror("Error - Auto Plan Breast 26Gy/5F v" + str(script_version), "No plan loaded. Please open a plan")
            exit()    
        ...
        try:
            ...
            top = Tk()
            top.withdraw()
            messagebox.showerror("Error - Auto Plan Breast 26Gy/5F v" + str(script_version), "No Beamset loaded. Please open a Beamset")
            exit()
     
        ##flag if dont select course, machine or breathing or CGs
        ...
        if clicked4.get() == 'Select Clinical Goal Template':
            top = Tk()
            top.withdraw()
            messagebox.showerror("Error - Auto Plan Breast v" + str(script_version), "Please Select Clinical Goal Template")
            exit()
        ...
        if len(beam_set.Beams) >1:
            top = Tk()
            top.withdraw()
            messagebox.showerror("Error - Auto Plan Breast 26Gy/5F v" + str(script_version), "Only one Medial field should exist when starting")
            exit()
        ...
        if beam_set.PlanGenerationTechnique != 'Conformal':
            top = Tk()
            top.withdraw()
            messagebox.showerror("Error - Auto Plan Breast 26Gy/5F v" + str(script_version), "Technique must be 3DCRT when starting")
            exit()
        ...
        if len(plan.BeamSets) >1:
            top = Tk()
            top.withdraw()
            messagebox.showerror("Error - Auto Plan Breast 26Gy/5F v" + str(script_version), "Only one Beamset should exist when starting")
            exit()
        ...
        if examination.PatientPosition != 'HFS':
            top = Tk()
            top.withdraw()
            messagebox.showerror("Error - Auto Plan Breast 26Gy/5F v" + str(script_version), "Script only for HFS!")
            exit() 
        ...
        if beam_set.Beams[0].Isocenter.Position.x > 0:
            for p in case.TreatmentPlans:
                if p.Name in ['Left Breast', 'Left P Breast']:
                    top = Tk()
                    top.withdraw()
                    messagebox.showerror("Error - Auto Plan Breast 26Gy/5F v" + str(script_version), "No Plan Name in Case can be called 'Left Breast'")
                    exit()
                    
        if beam_set.Beams[0].Isocenter.Position.x < 0:
            for p in case.TreatmentPlans:
                if p.Name in ['Right Breast', 'Right P Breast']:
                    top = Tk()
                    top.withdraw()
                    messagebox.showerror("Error - Auto Plan Breast 26Gy/5F v" + str(script_version), "No Plan Name in Case can be called 'Right Breast'")
                    exit()


        #checks if roi Skin AND ctv partial exits. if not, gives a error messege
        ...
        if 'Skin' not in roi_list:
            top = Tk()
            top.withdraw()
            messagebox.showerror("Error - Auto Plan Breast 26Gy/5F v" + str(script_version), "No ROI called 'Skin' found!")
            exit()
            
        if 'CTVp_Partial' not in roi_list:
            top = Tk()
            top.withdraw()
            messagebox.showerror("Error - Auto Plan Breast 26Gy/5F v" + str(script_version), "No ROI called 'CTVp_Partial' found!")
            exit()
        ...
        if 'CTVp_Partial' in roi_list:
            if roi_structures["CTVp_Partial"].HasContours() is False:
                top = Tk()
                top.withdraw()
                messagebox.showerror("Error - Auto Plan Breast v" + str(script_version), "ROI 'CTVp_Partial' needs outlining first!")
                exit()
                
        ##checks if ctv on other side to iso and flags warning incase planners beams not on partial breast ctv side
        CTVp_center = roi_structures["CTVp_Partial"].GetCenterOfRoi()
        center_CTVpPartial = {'x':CTVp_center.x, 'y':CTVp_center.y, 'z':CTVp_center.z}
        
        if beam_set.Beams[0].Isocenter.Position.x > 0:
            if CTVp_center.x < 0:
                top = Tk()
                top.withdraw()
                messagebox.showerror("Error - Auto Plan Breast 26Gy/5F v" + str(script_version), "'CTVp_Partial' found on opposite side to isocenter!")
                exit()
        ...
        ####PARTIAL BREAST - GROW GUIDE ROI PROV TO CHECK SUITABILITY AND THE NEW Y JAWS THAT WILL BE APPLIED LATER. ERROR FLAGS IF >11CM and varian OR shows messge if jaw > approved field jaw#################################################
        with CompositeAction('Expand (guide)'):

                retval_0 = case.PatientModel.CreateRoi(Name=r"guide", Color="Blue", Type="Ptv", TissueName=None, RbeCellTypeName=None, RoiMaterial=None)

                retval_0.SetMarginExpression(SourceRoiName=r"CTVp_Partial", MarginSettings={ 'Type': "Expand", 'Superior': 1.5, 'Inferior': 1.5, 'Anterior': 1.5, 'Posterior': 1.5, 'Right': 1.5, 'Left': 1.5 })

                retval_0.UpdateDerivedGeometry(Examination=examination, Algorithm="Auto")
                
        #
        #ISO POINT
        iso_z = beam_set.Beams[0].Isocenter.Position.z   ## iso z level value

        ss = case.PatientModel.StructureSets[examination.Name]

        guide_roi_box = ss.RoiGeometries['guide'].GetBoundingBox()

        z_min = guide_roi_box[0].z  ##most inf is negative value  -5.85
        z_max = guide_roi_box[1].z   ##most sup is positive value  4.05


        ##define rounding functions to nearest 0.5cm
        def roundup(x):
            return int(math.ceil(x / 0.5)) * 0.5
            
        def rounddown(x):
            return int(math.floor(x / 0.5)) * 0.5
            
        #sup distance 
        #sup_distance = roundup(z_max - iso_z + 0.3)  ##gives 4.05 +0.3 = 4.35##add 0.3cm as due to coll angle needs to be slightly bigger ##round to nearest 0.5cm jaw = 4.5cm
        if plan.Name == 'Left Breast':
            sup_distance = rounddown(z_max - iso_z + 0.2)  ##testing noticed too big sup on left side so rounddown
        if plan.Name == 'Right Breast':
            sup_distance = roundup(z_max - iso_z)

        #inf distance
        if plan.Name == 'Left Breast':
            inf_distance = rounddown(z_min - iso_z - 0.3)  #gives -5.85 - 0.3 = -6.15  ####add 0.3cm as due to coll angle needs to be slightly bigger # rounds to nearest 0.5cm jaw = -6.5cm
        if plan.Name == 'Right Breast':
            inf_distance = rounddown(z_min - iso_z - 0.3)

        #access jaws if mlc or not
        if len(beam_set.Beams[0].Segments) == 1:
            y1_jaw = beam_set.Beams[0].Segments[0].JawPositions[2]  #minus
            y2_jaw = beam_set.Beams[0].Segments[0].JawPositions[3]  #plus
        if len(beam_set.Beams[0].Segments) == 0:
            y1_jaw = beam_set.Beams[0].InitialJawPositions[2]  #minus
            y2_jaw = beam_set.Beams[0].InitialJawPositions[3]  #plus
            
        #raise ERROR messge if new jaw will be >11cm if clicked truebeam
        if clicked2.get() == 'TrueBeamSTx':
            if sup_distance > 11:
                case.PatientModel.RegionsOfInterest['guide'].DeleteRoi()
                top = Tk()
                top.withdraw()
                messagebox.showerror("Error - Auto Plan Breast 26Gy/5F v" + str(script_version), "Y2 Jaw will be > 11cm so plan on Elekta")
                exit()
            if inf_distance < -11:
                case.PatientModel.RegionsOfInterest['guide'].DeleteRoi()
                top = Tk()
                top.withdraw()
                messagebox.showerror("Error - Auto Plan Breast 26Gy/5F v" + str(script_version), "Y1 Jaw will be > 11cm so plan on Elekta")
                exit()
                
        #raise messge if new jaw will be > Dr jaw
        if sup_distance > y2_jaw:
            y_increase = "True" # added 20/4/2022
            top = Tk()
            top.withdraw()
            messagebox.showinfo("Error - Auto Plan Breast 26Gy/5F v" + str(script_version), "Note new Y2 Jaw will be > Y2 on checked field")
        else:
            y_increase = "False"
            
            

        if inf_distance < y1_jaw:
            y_increase = "True" # added 20/4/2022
            top = Tk()
            top.withdraw()
            messagebox.showinfo("Error - Auto Plan Breast 26Gy/5F v" + str(script_version), "Note new Y1 Jaw will be > Y1 on checked field")
        else:
            y_increase = "False"
        ...
        def changeMachine_NoMLC():
            ...
            beam_set = get_current("BeamSet")
            if len(beam_set.Beams[0].Segments) == 0:
                x1_jaw = beam_set.Beams[0].InitialJawPositions[0]
                x2_jaw = beam_set.Beams[0].InitialJawPositions[1]
                y1_jaw = beam_set.Beams[0].InitialJawPositions[2]  #minus
                y2_jaw = beam_set.Beams[0].InitialJawPositions[3]  #plus 


            #Beamset old parameters
            ...
            beam_set.Beams[0].SetInitialJawPositions(X1=x1_jaw,X2=x2_jaw,Y1=y1_jaw,Y2=y2_jaw) ##sets new jaws as we know the PB ones required
        ...
        def changeMachine_MLC():
            ...
            ##calc dose ##need to try and except as if grid size is default (not zero) when ctvs etc approved on prev plan then it cant make it a default grid.
            try:
        ...
        if len(poi_list) == 0:
            top = Tk()
            top.withdraw()
            messagebox.showerror("Error - Auto Plan Breast 26Gy/5F v" + str(script_version), "No POI with Type 'LocalizationPoint' defined!")
            exit()
        ...
        retval_0 = case.PatientModel.CreateRoi(Name=r"PTVp_2600", Color="Red", Type="Ptv", TissueName=None, RbeCellTypeName=None, RoiMaterial=None)
        ...
        with CompositeAction('ROI algebra (Skin-PTV)'):

          retval_2 = case.PatientModel.CreateRoi(Name=r"Skin-PTV", Color="Lime", Type="Organ", TissueName=None, RbeCellTypeName=None, RoiMaterial=None)

          retval_2.SetAlgebraExpression(ExpressionA={ 'Operation': "Union", 'SourceRoiNames': [r"Skin"], 'MarginSettings': { 'Type': "Expand", 'Superior': 0, 'Inferior': 0, 'Anterior': 0, 'Posterior': 0, 'Right': 0, 'Left': 0 } }, ExpressionB={ 'Operation': "Union", 'SourceRoiNames': [r"PTVp_2600"], 'MarginSettings': { 'Type': "Expand", 'Superior': 0, 'Inferior': 0, 'Anterior': 0, 'Posterior': 0, 'Right': 0, 'Left': 0 } }, ResultOperation="Subtraction", ResultMarginSettings={ 'Type': "Expand", 'Superior': 0, 'Inferior': 0, 'Anterior': 0, 'Posterior': 0, 'Right': 0, 'Left': 0 })
        ...
        retval_3 = case.PatientModel.CreateRoi(Name=r"xxBreast_T1(26Gy)", Color="Black", Type="Undefined", TissueName=None, RbeCellTypeName=None, RoiMaterial=None)


        #run MBS for lung ROIs
        if plan.Name == 'Left Breast':
        ...
        ##SORT BEAMS MED AND LAT DEPENDING IF THEY HAVE MLC OR NOT
        
            ###close leaves behind jaws - dont use conform mlc by beam_set.TreatAndProtect() as this removes potential heart mlc in place
        #works for elekta agility machine only
        ...
        def MakePBbeamsNOMLC():
            ...
            #name the beams first to define them and use them#Names beams med depending on machine name
            beam_set = get_current("BeamSet")
            if beam_set.MachineReference.MachineName == 'TrueBeamSTx':
                if plan.Name == 'Left Breast':
            ...
            #oppose gantry and align angle
            ##make gantry interger angle first BS0
            ...
            if plan.Name == 'Left Breast':
                x = 0.5 * abs(beam_set.Beams[0].InitialJawPositions[0]) * 2 / 100 ##x1 as a positive value eg -5.7 to 5.7
                divergence = math.degrees(math.atan(x))
            ...
            if plan.Name == 'Right Breast':
                x = 0.5 * abs(beam_set.Beams[0].InitialJawPositions[1]) * 2 / 100   ##x2 is post jaw positive on right breast
                divergence = math.degrees(math.atan(x))
            ...
            #OLD X JAWS
            Med_x1 = beam_set.Beams[0].InitialJawPositions[0]  
            Med_x2 = beam_set.Beams[0].InitialJawPositions[1]

            Lat_x1 = beam_set.Beams[1].InitialJawPositions[0]  
            Lat_x2 = beam_set.Beams[1].InitialJawPositions[1]

            ##add new beamset 1 and import beams from beamset 0 - same machine and 3dcrt #5
            plan.AddNewBeamSet(Name='getPBJaws', ExaminationName=examination.Name, MachineName=machine_name, Modality="Photons", TreatmentTechnique="Conformal", PatientPosition="HeadFirstSupine", NumberOfFractions=5, CreateSetupBeams=False, Comment="")
            ...
            ######################################################################################################################################################    
            #MAKE ROI d50_WB to later use to get oborders and thickness measurements. this is done now using full lenght fields before we reduce the j jaws
            #add mlc to beams
            beam_set.SetLeafPositioningThreshold(LeafPositioningThreshold=0)
            beam_set.TreatAndProtect()
            
            #add mus to beams and calc dose
            ##add mu
            ...
            #enter 242MU on each open beam_set calc dose
            beam_set = get_current("BeamSet")

            beam_set.Beams[0].BeamMU = 242
            beam_set.Beams[1].BeamMU = 242
            ...
            Skin_max_final = Skin_max[0]    #cGy max per plan dose
            ...
            case.PatientModel.StructureSets[examination.Name].RoiGeometries['d50_WB'].SetRepresentation(Representation='Contours')
            #################################################################################################################################
            
            
            ##set guide as treat roi for med and lat beams in this new beamset
            beam_set.SelectToUseROIasTreatOrProtectForAllBeams(RoiName = 'guide')

            ##set leaf positi threshold 0 and conform to guide
            beam_set.SetLeafPositioningThreshold(LeafPositioningThreshold=0)
            beam_set.TreatAndProtect()

            #record now new y jaws from this 2nd beamset
            Medy1 = beam_set.Beams[0].Segments[0].JawPositions[2] 
            Medy2 = beam_set.Beams[0].Segments[0].JawPositions[3]

            Laty1 = beam_set.Beams[1].Segments[0].JawPositions[2]
            Laty2 = beam_set.Beams[1].Segments[0].JawPositions[3]

            #set BS 0 as original now
            patient.Save()
            beam_set = plan.BeamSets[0]
            beam_set.SetCurrent()

            #set intial jaw positionins now
            beam_set.Beams[0].SetInitialJawPositions(X1=Med_x1,X2=Med_x2,Y1=Medy1,Y2=Medy2)  #med
            beam_set.Beams[1].SetInitialJawPositions(X1=Lat_x1,X2=Lat_x2,Y1=Laty1,Y2=Laty2)  #lat
            ...
            ##now can conform mlc to jaws and so no need to close leafs outside jaws in future
            beam_set.SetLeafPositioningThreshold(LeafPositioningThreshold=0)
            beam_set.TreatAndProtect() 
        ##################################################################################################################
        ...
        def MakePBbeamsMLC():
            ...
            #OLD X JAWS
            Med_x1 = beam_set.Beams[0].Segments[0].JawPositions[0]  
            Med_x2 = beam_set.Beams[0].Segments[0].JawPositions[1]

            Lat_x1 = beam_set.Beams[1].Segments[0].JawPositions[0]  
            Lat_x2 = beam_set.Beams[1].Segments[0].JawPositions[1]
            ...
            #Need to delete or remove zzd50 roi as it copies over the treat margins? remove treat or protect roi function at beam level first
            beam_set.Beams[0].RemoveTreatOrProtectRoi(RoiName = 'zzd50')
            beam_set.Beams[1].RemoveTreatOrProtectRoi(RoiName = 'zzd50')
            ...
            #set new jaw positionins now
            for beam in beam_set.Beams:
                segments = beam.Segments[0]
                jaw = segments.JawPositions
                if beam.Name in ['EA01', 'VA01']: 
                    jaw[2] = Medy1
                    jaw[3] = Medy2
            ...
            #####depending on which machine determines script to close leaves behind jaws
            if beam_set.MachineReference.MachineName == 'Agility':
        ...
        ##decide how to make PB med and lat beam based if has mlc or not
        beam_set = get_current("BeamSet")
        if len(beam_set.Beams[0].Segments) == 0:
            MakePBbeamsNOMLC()
        else:  ##this run assume MLC
            MakePBbeamsMLC()

        #########this means if the new y jaws need to increase beyond the checkers jaw that we need to conform mlc to field now in case the top or bottom mlcs remain within the field
        if y_increase == "True":
            beam_set.SetLeafPositioningThreshold(LeafPositioningThreshold=0)
            beam_set.TreatAndProtect()
        ...
        with CompositeAction('ROI algebra (PTVp_2600)'):

          retval_2 = case.PatientModel.RegionsOfInterest['PTVp_2600'].SetAlgebraExpression(ExpressionA={ 'Operation': "Union", 'SourceRoiNames': [r"2"], 'MarginSettings': { 'Type': "Contract", 'Superior': 0.5, 'Inferior': 0.5, 'Anterior': 0.5, 'Posterior': 0.5, 'Right': 0.5, 'Left': 0.5 } }, ExpressionB={ 'Operation': "Union", 'SourceRoiNames': [], 'MarginSettings': { 'Type': "Expand", 'Superior': 0, 'Inferior': 0, 'Anterior': 0, 'Posterior': 0, 'Right': 0, 'Left': 0 } }, ResultOperation="None", ResultMarginSettings={ 'Type': "Expand", 'Superior': 0, 'Inferior': 0, 'Anterior': 0, 'Posterior': 0, 'Right': 0, 'Left': 0 })
        ...
        ##with CompositeAction('Delete ROI (d50)'):

          ##case.PatientModel.RegionsOfInterest['d50'].DeleteRoi()

          # CompositeAction ends 


        with CompositeAction('Update derived geometry (Skin-PTV)'):

          case.PatientModel.RegionsOfInterest['Skin-PTV'].UpdateDerivedGeometry(Examination=examination, Algorithm="Auto")
        ...
        if plan.Name == 'Left Breast':
            ...
            ss = case.PatientModel.StructureSets[examination.Name]
            for [contour_index, contour] in enumerate(ss.RoiGeometries['d50_WB'].PrimaryShape.Contours):
        ...
        machine_name = plan.BeamSets[0].MachineReference.MachineName
        plan.AddNewBeamSet(Name='copy', ExaminationName=examination.Name, MachineName=machine_name, Modality="Photons", TreatmentTechnique="SMLC", PatientPosition="HeadFirstSupine", NumberOfFractions=5, CreateSetupBeams=False, Comment="")
        ...
        plan.BeamSets[0].AddRoiPrescriptionDoseReference(RoiName="PTVp_2600", PrescriptionType="MedianDose", DoseValue=2600, RelativePrescriptionLevel=1.0)
        ...
        beam_set.SelectToUseROIasTreatOrProtectForAllBeams(RoiName = 'PTVp_2600')

        ###need to covert PTV to contour type to enable it to have contours as an objext in primary shape
        case.PatientModel.StructureSets[examination.Name].RoiGeometries['PTVp_2600'].SetRepresentation(Representation='Contours')
        ...
        if plan.Name == 'Right Breast':
            ...
            roi_center = roi_geometries['PTVp_2600'].GetCenterOfRoi()
            ...
            ss = case.PatientModel.StructureSets[examination.Name]
            for [contour_index, contour] in enumerate(ss.RoiGeometries['PTVp_2600'].PrimaryShape.Contours):
                for coordinate in contour:
                    ...
                    if coordinate.z <= roi_center.z + 0.6 and coordinate.z >= roi_center.z - 0.3 and coordinate.y <= roi_center.y + 0.6 and coordinate.y >= roi_center.y - 0.3:
        ...
        #
            ...
            cwallMednew_x = max(cwallx_coords)  ##(for RIGHT breast we then want largest x which is cloest to medial side)

            case.PatientModel.CreatePoi(Examination=examination, Point={ 'x': cwallLatnew_x, 'y': roi_center.y, 'z': roi_center.z }, Name=r"lat cwall", Color="Green", VisualizationDiameter=1, Type="Undefined")  ##lat poi right breast
            case.PatientModel.CreatePoi(Examination=examination, Point={ 'x': cwallMednew_x, 'y': roi_center.y, 'z': roi_center.z }, Name=r"med cwall", Color="Green", VisualizationDiameter=1, Type="Undefined")  ##med poi right breast
            ...
            if thickness_PTVp2600 <= 4.0:
                beam_set.Beams[2].SetTreatAndProtectMarginsForBeam(TopMargin=0.4, BottomMargin=0.4, LeftMargin=2.0, RightMargin=0.4, Roi='PTVp_2600')
                beam_set.Beams[3].SetTreatAndProtectMarginsForBeam(TopMargin=0.4, BottomMargin=0.4, LeftMargin=0.4, RightMargin=2.0, Roi='PTVp_2600')
            elif thickness_PTVp2600 > 4.0:
                beam_set.Beams[2].SetTreatAndProtectMarginsForBeam(TopMargin=0.4, BottomMargin=0.4, LeftMargin=0.4, RightMargin=0.4, Roi='PTVp_2600')
                beam_set.Beams[3].SetTreatAndProtectMarginsForBeam(TopMargin=0.4, BottomMargin=0.4, LeftMargin=0.4, RightMargin=0.4, Roi='PTVp_2600')
        ...
        #
            ...
            if thickness_PTVp2600 <= 4.0:
                beam_set.Beams[2].SetTreatAndProtectMarginsForBeam(TopMargin=0.4, BottomMargin=0.4, LeftMargin=0.4, RightMargin=2.0, Roi='PTVp_2600')
                beam_set.Beams[3].SetTreatAndProtectMarginsForBeam(TopMargin=0.4, BottomMargin=0.4, LeftMargin=2.0, RightMargin=0.4, Roi='PTVp_2600')
            elif thickness_PTVp2600 > 4.0:
                beam_set.Beams[2].SetTreatAndProtectMarginsForBeam(TopMargin=0.4, BottomMargin=0.4, LeftMargin=0.4, RightMargin=0.4, Roi='PTVp_2600')
                beam_set.Beams[3].SetTreatAndProtectMarginsForBeam(TopMargin=0.4, BottomMargin=0.4, LeftMargin=0.4, RightMargin=0.4, Roi='PTVp_2600')
        ...
        #add manual opt functions as issues with loading templates source/top
        with CompositeAction('Add optimization function'):

          retval_0 = plan.PlanOptimizations[0].AddOptimizationFunction(FunctionType="MinDose", RoiName=r"PTVp_2600", IsConstraint=False, RestrictAllBeamsIndividually=False, RestrictToBeams=None, IsRobust=False, RestrictToBeamSet=None, UseRbeDose=False)

          plan.PlanOptimizations[0].Objective.ConstituentFunctions[0].DoseFunctionParameters.DoseLevel = 2500
        ...
        with CompositeAction('Add optimization function'):
          ...
          retval_1 = plan.PlanOptimizations[0].AddOptimizationFunction(FunctionType="MaxDose", RoiName=r"PTVp_2600", IsConstraint=False, RestrictAllBeamsIndividually=False, RestrictToBeams=None, IsRobust=False, RestrictToBeamSet=None, UseRbeDose=False)

          plan.PlanOptimizations[0].Objective.ConstituentFunctions[1].DoseFunctionParameters.DoseLevel = 2700
        ...
        with CompositeAction('Add optimization function'):
          ...
          retval_2 = plan.PlanOptimizations[0].AddOptimizationFunction(FunctionType="UniformDose", RoiName=r"PTVp_2600", IsConstraint=False, RestrictAllBeamsIndividually=False, RestrictToBeams=None, IsRobust=False, RestrictToBeamSet=None, UseRbeDose=False)

          plan.PlanOptimizations[0].Objective.ConstituentFunctions[2].DoseFunctionParameters.DoseLevel = 2600
        ...
        with CompositeAction('Add optimization function'):
          ...
          plan.PlanOptimizations[0].Objective.ConstituentFunctions[3].DoseFunctionParameters.HighDoseLevel = 2600

          plan.PlanOptimizations[0].Objective.ConstituentFunctions[3].DoseFunctionParameters.LowDoseLevel = 2400
        ...
        case.CaseSettings.DoseColorMap.ReferenceValue = 2600
        ########################################################################################
        ...
        PTV_v95 = beam_set.FractionDose.GetRelativeVolumeAtDoseValues(RoiName='PTVp_2600', DoseValues=[dose_value_v95])*100  ##this gets v24.7 in 5# = 494 * 100 = 95.22 (as was given as 0.9522) # tested in console
        PTV_v105 = beam_set.FractionDose.GetRelativeVolumeAtDoseValues(RoiName='PTVp_2600', DoseValues=[dose_value_v105])*100
        PTV_v107 = beam_set.FractionDose.GetRelativeVolumeAtDoseValues(RoiName='PTVp_2600', DoseValues=[dose_value_v107])*100
        ...
        PTV_volume = plan.TreatmentCourse.TotalDose.GetDoseGridRoi(RoiName = 'PTVp_2600').RoiVolumeDistribution.TotalVolume
        PTV_v110_rel = beam_set.FractionDose.GetRelativeVolumeAtDoseValues(RoiName='PTVp_2600', DoseValues=[dose_value_v110])
        PTV_v110_abs = PTV_v110_rel[0]*PTV_volume

        #Skin-PTV_v107cc # tested console gives absolute volume result
        SkinPTV_volume = plan.TreatmentCourse.TotalDose.GetDoseGridRoi(RoiName = 'Skin-PTV').RoiVolumeDistribution.TotalVolume
        SkinPTV_v107_rel = beam_set.FractionDose.GetRelativeVolumeAtDoseValues(RoiName='Skin-PTV', DoseValues=[dose_value_v107])
        SkinPTV_v107_abs = SkinPTV_v107_rel[0]*SkinPTV_volume

        #Skin-PTV_v110cc #tested console gives absolute volume result
        SkinPTV_volume = plan.TreatmentCourse.TotalDose.GetDoseGridRoi(RoiName = 'Skin-PTV').RoiVolumeDistribution.TotalVolume
        SkinPTV_v110_rel = beam_set.FractionDose.GetRelativeVolumeAtDoseValues(RoiName='Skin-PTV', DoseValues=[dose_value_v110])
        ...
        patient.SetRoiVisibility(RoiName='PTVp_2600', IsVisible=True)
        patient.SetRoiVisibility(RoiName='Skin', IsVisible=False)
        patient.SetRoiVisibility(RoiName='Lung_Ipsi', IsVisible=False)
        patient.SetRoiVisibility(RoiName='Skin-PTV', IsVisible=False)
        patient.SetRoiVisibility(RoiName='xxBreast_T1(26Gy)', IsVisible=False)
        patient.SetRoiVisibility(RoiName='d50', IsVisible=False)
        patient.SetRoiVisibility(RoiName='guide', IsVisible=False)
        
        #delete the d50_WB roi as not needed
        case.PatientModel.RegionsOfInterest['d50_WB'].DeleteRoi()
        ...
        ##sort plan name
        if plan.Name == 'Left Breast':
            plan.Name = 'Left P Breast'
            plan.BeamSets[0].DicomPlanLabel = plan.Name
            
        if plan.Name == 'Right Breast':
            plan.Name = 'Right P Breast'
            plan.BeamSets[0].DicomPlanLabel = plan.Name
            
        ##update skin-ptv roi status prior to approval
        case.PatientModel.RegionsOfInterest['Skin-PTV'].UpdateDerivedGeometry(Examination=examination, Algorithm="Auto")
        ...
        try:
            ...
            MyFileName = patient.PatientID + '_PB_' + str(staff_id) + '.txt'
            file = open(MyFileName, "w") # write mode over writes
            file.write("PB Script ran - start time: " + str(startTime)
            ...
            messagebox.showinfo('Auto Plan Breast 26Gy/5F v' + str(script_version), 'log file not created - inform scripting of this instance')
    ...
    myButton2 = Button(top, text="Start PARTIAL BREAST Plan 26Gy/5F", command=autoplanPBF)
